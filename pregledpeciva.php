...
//radi kako treba i koristim ovaj file
...
$path = "images/".$picture;
//create sql
$sql = "INSERT INTO pecivaipalacinke (title, ingredients, price, picture) VALUES('$title', '$ingredients', '$price', '$path')";
...
$sql = 'SELECT title, ingredients, price, picture, id FROM pecivaipalacinke ORDER BY id DESC';

//make query and get result
$result = mysqli_query($connection, $sql);

//fetch the resulting rows as an array
$peciva = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
if(isset($_POST['obrisati'])){
	...
	$sql = "DELETE FROM pecivaipalacinke WHERE id = $id_to_delete";
...
if(isset($_GET['id'])){
	...
	//make sql
	$sql = "SELECT *  FROM pecivaipalacinke WHERE id = $id";
	
	//get the query result
	$result = mysqli_query($connection, $sql);
	
	//fetch result in array format
	$buns = mysqli_fetch_assoc($results);
...
<nav class="navbar navbar-expand-sm px-0">
    <a id="hover" class="navbar-brand font-weight-bold px-3 rounded-pill d-none d-md-block" href="index.php">&nbsp;Beksef<br>Slastice</a>
      ...
      <a class="text-decoration-none" href="index.php" data-toggle="popover" data-placement="left" data-trigger="hover" data-content="Početna"><h1 class="font-weight-bold" id="navbarpregleda">Peciva</h1>  </a>  
</div>
</div>
      <a id="hover" class="navbar-brand font-weight-bold px-3 rounded-pill d-none d-md-block" href="index.php">&nbsp;Beksef<br>Slastice</a>
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content text-color">
              <div class="modal-header">
                <h5 class="modal-title font-weight-bold fontmenu">Peciva</h5>
              </div>
...
<form class="needs-validation" novalidate method="post">
  <div class="form-row">
    ...
    <div class="col-md-12 mb-3">
      <label for="validationCustomUsername">Ponuda</label>
      <div class="input-group">
        
        <input type="text" class="form-control" id="validationCustomUsername" placeholder="ponuda"
    ...
    <div class="row">
        ...
        <div class="col-md-4 pb-4">
                ...
                <div class="card rounded font-italic pt-4" id="okvirkarticeupregledu">
                    ...
                    <img id="velicina_slika" class="img-fluid mx-auto d-block rounded-pill shadow okvirslike" src="<?php echo ($pecivo['picture']);?>" alt="cokotorta">
                    
                <div class="card-body velicina_kartice">
                      
                      <h5 class="text-center font-weight-bold"><?php echo htmlspecialchars($pecivo['title']);?></h5>
                      <p class="card-text">Ponuda:</p>
                      <ul>
                            <?php foreach(explode(',',$pecivo['ingredients']) as $ing):?>
                            <li>
                ...
                <div class="card-footer boja_footer">
                      <p class="text-right font-weight-bold"><?php echo htmlspecialchars($pecivo['price']);?></p>
                      
                      </div>
                <!-- delete form -->
		<form action="pregledpeciva.php" method="POST" class="text-center mt-3 mb-3">
			<input type="hidden" name="id_to_delete" value="<?php echo $pecivo['id']?>">
